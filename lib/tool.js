...
function createCouncilRecorder(ctx) {
	...
	ctx.on("workflow/phase", (info, title) => {
		...
		if (entry === void 0 || entry.phases.length >= MAX_PHASES) return;
		entry.phases.push({
			title,
			at: Date.now()
		});
	...
	ctx.on("workflow/log", (info, message) => {
		const entry = active.get(info.id);
		if (entry === void 0 || entry.messages.length >= MAX_MESSAGES) return;
		entry.messages.push({
			text: message,
			at: Date.now()
		});
	...
	return {
		start(session, run) {
			const opened = append(session, "tool-workflow/run-start", {
				runId: run.id,
				name: run.meta.name
			});
			active.set(run.id, {
		...
		narration(runId) {
			const entry = active.get(runId);
			if (entry === void 0) return NO_NARRATION;
			return {
...
*/
function sessionSetupOf(section, sessionId) {
...
*/
function applySessionSetup(preset, setup, maxAgentsPerLayer) {
	if (setup === void 0) return preset;
	const tunings = setup.roles ?? {};
	/** One preset role with its session tuning applied. */
	const tuneRole = (layerId) => (role) => {
		const tune = tunings[`${layerId}.${role.id}`];
		if (tune === void 0) return role;
		return {
			...role,
			...tune.count === void 0 ? {} : { count: tune.count },
			...tune.model === void 0 || tune.model === "" ? {} : { model: tune.model },
			...tune.provider === void 0 || tune.provider === "" ? {} : { provider: tune.provider },
			...tune.prompt === void 0 || tune.prompt.trim() === "" ? {} : { prompt: tune.prompt }
		};
	};
	...
	const authorRole = (role) => ({
		...
		label: role.label ?? role.id,
		prompt: role.prompt,
		...role.count === void 0 ? {} : { count: role.count },
		...role.model === void 0 || role.model === "" ? {} : { model: role.model },
		...role.provider === void 0 || role.provider === "" ? {} : { provider: role.provider }
	});
	const authorOf = (layerId) => setup.addRoles?.[layerId] ?? [];
	const kept = preset.layers.filter((layer) => !layerDropped(layer.kind, setup)).map((layer) => {
		if (layer.kind === "reduce") {
			for (const role of layer.roles) {
				const tune = tunings[`${layer.id}.${role.id}`];
				if (tune?.count !== void 0 && tune.count !== 1) throw new RangeError(`council: the reduce role "${role.id}" always runs exactly one instance; this session requested ${tune.count}`);
			}
			return {
				...layer,
				roles: layer.roles.map(tuneRole(layer.id))
			};
		}
		const tuned = layer.roles.map(tuneRole(layer.id));
		const authored = layer.kind === "map" || layer.kind === "verify" ? authorOf(layer.id).map(authorRole) : [];
		return authored.length === 0 ? {
			...layer,
			roles: tuned
		} : {
			...layer,
			roles: [...tuned, ...authored]
		};
	});
	...
	for (const layer of kept) {
		const width = layer.roles.reduce((sum, role) => sum + (role.count ?? 1), 0);
		if (width > maxAgentsPerLayer) throw new RangeError(`council: this session's setup starts ${width} members on layer "${layer.id}" of preset "${preset.id}"; this deployment allows at most ${maxAgentsPerLayer} per layer. Narrow a role in the council designer.`);
		if (layer.kind !== "verify") continue;
		if (setup.quorum === void 0 || Object.keys(setup.quorum).length === 0) continue;
		const rule = setup.quorum.rule ?? "majority";
		if (rule === "threshold") {
			const threshold = setup.quorum.threshold ?? width;
			if (!Number.isInteger(threshold) || threshold < 1 || threshold > width) throw new RangeError(`council: preset "${preset.id}" verify layer needs a threshold between 1 and its width ${width}; this session set ${String(threshold)}`);
			layer.quorum = {
				rule,
				threshold
			};
		} else layer.quorum = { rule };
	}
	return {
		...preset,
		layers: kept
...
*/
function applyCustomSetup(layers, maxAgentsPerLayer, name) {
	...
	const presetLayers = layers.map((layer) => {
		...
		if (width > maxAgentsPerLayer) throw new RangeError(`council: this custom session starts ${width} members on layer "${layer.id}"; this deployment allows at most ${maxAgentsPerLayer} per layer. Narrow a role in the council designer.`);
		if (kind === "verify") {
			const quorum = layer.quorum ?? {};
			const rule = quorum.rule ?? "majority";
			if (rule === "threshold") {
				const threshold = quorum.threshold ?? width;
				if (!Number.isInteger(threshold) || threshold < 1 || threshold > width) throw new RangeError(`council: custom verify layer "${layer.id}" needs a threshold between 1 and its width ${width}; this session set ${String(threshold)}`);
				return {
					id: layer.id,
					kind,
					label: layer.label ?? layer.id,
	...
	return {
		...
		framing: "A council of independent agents is working the task below. You are one member: you share the workspace but no conversation. Read the workspace, run what you can, and report only what you verified yourself.",
...
function presentResult(args, result) {
	...
	if (view.durationMs > 0) parts.push(`${Math.round(view.durationMs / 1e3)}s`);
...
function readArtifact(meta) {
	...
	if (meta["kind"] !== "council-run") return void 0;
	if (meta["version"] !== 1) return void 0;
	if (typeof meta["preset"] !== "string" || typeof meta["runId"] !== "string") return void 0;
	if (!isRecord(meta["counts"]) || !Array.isArray(meta["rows"])) return void 0;
	return meta;
}
/**
...
function apply(ctx, config) {
	...
	const readSettings = () => {
		return ctx.get("settings")?.get(COUNCIL_NAMESPACE);
	};
	const presetList = composed.presets.map((preset) => `- ${preset.id}: ${preset.description}`).join("\n");
	const presetGuidance = "Pick the preset by the task — `bug-hunt` for finding defects or auditing code, `research` for investigating a question, `feature-design` for designing a feature, `refactor` for planning a refactor. Its verdicts are what its members reported, not independent certification.";
	ctx.systemPrompt.section({
		name: "tool:council",
		order: 117,
		text: composed.councilEveryRequest ? "You are operating in Map-Reduce mode. Answer every substantive request through the `council` tool: choose the preset that matches the task, call `council` with the full task text, then report its verdict table and written conclusion. Only trivial chit-chat may be answered directly. " + presetGuidance : "The `council` tool runs a fan-out of independent subagents over one task. Reach for it when a question is worth several independent readings — an audit, a design decision, a claim you want cross-checked — and answer directly otherwise. " + presetGuidance
	});
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			preset: {
				...
				description: `Topology to run. Choose by the task: bug-hunt for finding defects/auditing code, research for investigating a question, feature-design for designing a feature, refactor for planning a refactor. Defaults to ${composed.defaultPreset.id}. Ignored when this session has a council configured in the composer designer — that topology is fixed for the session and runs for every request in it.`
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			const setup = sessionSetupOf(section, parent.session.id);
			let runPreset;
			if ((setup?.topology ?? []).length > 0 && (setup?.presetId ?? "") === "") {
				runPreset = applyCustomSetup(setup.topology ?? [], resolved.maxAgentsPerLayer, setup?.name ?? "");
				runPreset = resolveConfig({
					presets: [runPreset],
					defaultPreset: runPreset.id,
					maxAgentsPerLayer: resolved.maxAgentsPerLayer,
					maxLayers: resolved.maxLayers
				}).presets[0] ?? runPreset;
			} else if (setup?.presetId !== void 0 && setup.presetId !== "") {
				const fixed = resolved.presets.find((candidate) => candidate.id === setup.presetId);
				if (fixed === void 0) throw new Error(`council: this session's preset "${setup.presetId}" is not offered by the deployment`);
				...
				const picked = args.preset === void 0 ? resolved.defaultPreset : resolved.presets.find((candidate) => candidate.id === args.preset);
				if (picked === void 0) throw new Error(`council: unknown preset "${String(args.preset)}"`);
				runPreset = picked;
			}
			...
			try {
				...
				if (outcome.tally !== null) {
					const quorum = runPreset.layers.find((layer) => layer.kind === "verify")?.quorum ?? { rule: "majority" };
...
//#endregion
export { BUILTIN_PRESETS, COUNCIL_ARTIFACT_KIND, COUNCIL_ARTIFACT_VERSION, COUNCIL_NAMESPACE, Config, HARD_STOP_GRACE_MS, TABLE_LEGEND, TASK_SNIPPET_CHARS, apply, applyQuorum, applySessionSetup, assertClustersWellFormed, buildResultRecord, capPerMember, dedupeFindings, expandLayers, failureRecord, fingerprint, inject, mergeClusters, name, normalizeLocation, presentCall, presentResult, readArtifact, readOutcome, renderOutcome, renderTable, resolveConfig, sessionSetupOf, stopReasonError, summaryLine, tally, taskSnippet, totalAgentBudget };
