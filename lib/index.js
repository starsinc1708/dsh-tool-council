...
const QuorumOverrideSchema = z.object({
	rule: z.union([
		"majority",
		"unanimous",
		"threshold"
...
*/
function registeredProviders(ctx) {
	try {
		return ctx.get("subagents")?.list() ?? [];
	} catch {
...
function apply(ctx, config) {
	...
	installSettingsSection(ctx, settingsNamespace(COUNCIL_NAMESPACE), CouncilSettingsSchema, {
		...
		onChange: () => {},
		validate: (value) => {
			if (value.maxAgentsPerLayer !== void 0 && value.maxAgentsPerLayer !== policy.maxAgentsPerLayer) throw new TypeError("council: maxAgentsPerLayer mirrors the composition and cannot be set here");
			if (value.agentPresetId !== void 0 && value.agentPresetId !== presetId) throw new TypeError("council: agentPresetId mirrors the composition and cannot be set here");
			if (value.topology !== void 0 && JSON.stringify(value.topology) !== baseTopologyJson) throw new TypeError("council: topology mirrors the composition and cannot be set here");
			if (value.availableProviders !== void 0 && JSON.stringify(value.availableProviders) !== JSON.stringify(registeredProviders(ctx))) throw new TypeError("council: availableProviders mirrors the composition and cannot be set here");
			resolveConfig({
...
//#endregion
export { Config, CouncilSettingsSchema, apply, composeRows, name };
