...
const COUNCIL_EVENTS = /* @__PURE__ */ new Set([
...
const councilLogDefinition = {
	...
	match(event) {
		...
		return {
			...
			role: event.type === "tool-council/run-start" ? "start" : "update"
		};
	},
	start(context, match) {
	...
	update(context, match) {
		...
		if (match.event.type === "tool-council/phase") {
			const { title, at } = match.event.data;
			return {
				...state,
				phases: [...state.phases, {
					title,
		...
		const { message, at } = match.event.data;
		return {
			...state,
...
function CouncilView(props) {
	...
	else if (node.kind === "council-log") logs.set(node.id, node.data);
...
function Run({ run, defaultOpen, costRate, t, useMemberUsage, useLayerTokens }) {
	...
	const layers = new Map((log?.layers ?? []).map((layer) => [layer.id, layer]));
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("summary", {
			...
			children: [
				...
				log === null || log.task === "" ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				...
				log === null || log.startedAt === 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: council_view_module_css_default.runMeta,
					children: t("startedAt", { time: formatTime(log.startedAt) })
				}),
			...
			children: [
				...
				log === null || log.messages.length === 0 ? null : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
					className: council_view_module_css_default.runLog,
					children: log.messages.map((line, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: line.text }, index))
				}),
...
*/
function phaseDuration(log, title, endedAt) {
